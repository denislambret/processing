...
public class Star {
	...
	public Star(PApplet p, PVector location, int n, float w, float h, float startAngle, float proportion) {
		this.p = p;
		this.nbpoints = n;
	...
	public Star(PApplet p, PVector location, int n, float radius) {
		this.p = p;
		this.nbpoints = n;
	...
	public void show() {
		...
		for (int i = 0; i < 2 * nbpoints; i++) {
			...
			//p.rect(dw * (float) Math.cos(startAngle + angle * i), dh * (float) Math.sin(startAngle + angle * i), 150,
				//	150);
		}
		p.popMatrix();
	}
